...
function ScanContent() {
  ...
  const handleBarcodeScanned = (barcode: string) => {
    ...
    (window as any).__lastBarcode = barcode;
  ...
  const addProduct = async () => {
    if (!productName.trim() || !sessionId) return;
    
    const barcode = (window as any).__lastBarcode;
    if (!barcode) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLastScanned(null);
        (window as any).__lastBarcode = null;
  ...
  const submitAll = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(error.message || 'Failed to add products');
      }
    } catch (err) {
              ...
              <div className="flex-1 space-y-4">
                <div>
                  ...
                  <div className="bg-muted/50 border border-border rounded-lg px-3 py-2 text-foreground font-mono">
                    {(window as any).__lastBarcode || 'Scanning...'}
                  </div>
                </div>
